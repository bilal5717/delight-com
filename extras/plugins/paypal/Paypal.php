...
class Paypal extends Payment
{
	...
	{
		// Set URLs
		parent::$uri['previousUrl'] = str_replace(['#entryId'], [request()->ids], parent::$uri['previousUrl']);
		parent::$uri['nextUrl'] = str_replace(['#entryId'], [request()->ids], parent::$uri['nextUrl']);
		parent::$uri['paymentCancelUrl'] = str_replace(['#entryId'], [request()->ids], parent::$uri['paymentCancelUrl']);
		parent::$uri['paymentReturnUrl'] = str_replace(['#entryId'], [request()->ids], parent::$uri['paymentReturnUrl']);
		...
		$legnth = count(explode(',', $request->ids));
		// API Parameters
		$providerParams = [
			...
			'amount'      => Number::toFloat($package->price * $legnth),
		...
		try {
			...
			$localParams['transaction_id'] = $response->getTransactionReference();
			...
			if ($response->isSuccessful()) {
				...
				return self::upgradePost($localParams, $post);
