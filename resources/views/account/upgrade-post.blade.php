                        ...
                        <h3>
                            <?php
                                if (auth()->check()) {
                                    ...
                                    { 
                                        ?>
                            <strong class="text-default" style="font-size: 18px"> {{ t('my_ads') }} - </strong>
                            <br>
                            <ul>
                                ...
                                <li>
                                    <?php $postLink ='
                                            ...
                                            class="tooltipHere" title="" data-placement="top" data-toggle="tooltip" 
                                            data-original-title="' .$post->title . '">' . \Illuminate\Support\Str::limit($post->title, 45) . '
                                            </a>';
                                            echo  $key+1 . ')' . $postLink  ;
                                        } 
                                    ?>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <form class="form" id="postForm" method="POST"
                                    action="{{ url('account/my-posts/' . request()->ids . '/payment') }}">
                                    {!! csrf_field() !!}

                                    <input type="hidden" name="post_ids" value="{{ request()->ids }}">
                                    <fieldset>
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-12 text-center mt-4">
                                                @if (request()->segment(2) == 'create')
                                                    <a id="skipBtn"
                                                        href="{{ url('posts/create/' . $post->tmp_token . '/finish') }}"
                                                        class="btn btn-default btn-lg">
                                                        {{ t('Skip') }}
                                                    </a>
                                                @else
                                                    <a id="skipBtn" href="{{ url('account/my-posts') }}"
                                                        class="btn btn-default btn-lg">
                                                        {{ t('Skip') }}
                                                    </a>
                                                @endif
    ...
    <script>
        @if (isset($packages) && isset($paymentMethods) && $packages->count() > 0 && $paymentMethods->count() > 0)
            ...
            $(document).ready(function() {
                ...
                var packagePrice = (ele.length * getPackagePrice(selectedPackage)).toFixed(2);
                ...
                $('.package-selection').click(function() {
                    ...
                    packagePrice = (ele.length * getPackagePrice(selectedPackage)).toFixed(2);
        ...
        function showPaymentSubmitButton(
            currentPackagePrice,
            packagePrice,
            currentPaymentIsActive,
            paymentMethod,
