                            ...
                            <h4 class="card-title">
                                <div class="row">
                                    <div class="col-9 mt-2">
                                        <a href="#addressPanel" class="d-inline">{{ t('shipping_address') }}</a>
                                    </div>
                                    <div class="col-3 float-right">
                                        <a class="btn-add-listing  m-1 p-1"
                                            href="{{ route('create_shipping_address') }}">{{ t('add_new_address') }}
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-md-12">
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                <th scope="col">{{ t('address_title') }}</th>
                                                    <th scope="col">{{ t('address') }}</th>
                                                    <th scope="col">{{ t('company_city') }}</th>
                                                    <th scope="col">{{ t('company_country') }}</th>
                                                    <th scope="col">{{ t('default_address') }}</th>
                                                ...
                                                @foreach ($addresses as $address)
                                                    ...
                                                    <td>
                                                            <span data-toggle="tooltip" data-bs-placement="top"
                                                                ...
                                                                {{ Str::limit($address->address_title, 50) }}
                                                            </span>
                                                        </td>
                                                        <td>
                                                            <span data-toggle="tooltip" data-bs-placement="top"
                                                        ...
                                                        <td class="d-flex ">
                                                            <a class="btn btn-sm btn-primary mx-1" data-toggle="tooltip"
                                                                data-bs-placement="bottom" title="{{t("Edit")}}"
                                                                href="{{ route('edit_shipping_address', ['id' => $address->id]) }}">
                                                                <i class="far fa-edit fa-lg"></i>
                                                            </a>
                                                            <a class="delete-address btn btn-sm btn-danger"
                                                                data-toggle="tooltip" data-bs-placement="bottom"
                                                                title="{{t("Delete")}}" data-id="checkbox-{{ $address->id }}"
                                                                href="{{ route('delete_shipping_address', ['id' => $address->id]) }}"><i
...
@section('after_scripts')
    ...
    <script>
        $(document).ready(function() {
            const totalAddresses = "{{count($addresses)}}";
            console.log();
            $(".switchboxes").change(function() {
                if (!$(this).prop('checked') && totalAddresses == 1) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Oops...',
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('update-default-shipping-address') }}",
                    data: {
        ...
        $('.delete-address').on('click', function(e) {
            ...
            if ($('.' + id).prop('checked')) {
                Swal.fire({
                    icon: 'warning',
                    title: "{{ t('Oops...') }}",
                    text: "{{ t('You can not delete default selected address!') }}",
                });
                return false;
            }
