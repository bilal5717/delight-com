                                ...
                                <tr>
                                            ...
                                            @if ($plugin->installed and $plugin->activated)
                                                @if (!empty($plugin->options))
                                                    @foreach ($plugin->options as $option)
                                                        ...
                                                        <a class="btn btn-xs {{ isset($option->btnClass) && !empty($option->btnClass) ? $option->btnClass : 'btn-light' }}"
                                                            href="{{ $option->url }}">
                                                            <i
                                                                class="{{ isset($option->iClass) && !empty($option->iClass) ? $option->iClass : 'fa fa-cog' }}"></i>
                                    ...
                                    <td class="text-right">
                                        @if ($plugin->has_installer)
                                            @if ($plugin->installed)
                                                @if ($plugin->activated)
                                                    <a class="btn btn-xs btn-success btn-uninstall"
                ...
                <form role="form" method="POST" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group required <?php echo (isset($errors) and $errors->has('purchase_code_valid')) ? ' is-invalid' : ''; ?>">
                            ...
                            <input id="purchaseCode" name="purchase_code" class="form-control required"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary"
                            id="btnSubmit">{{ trans('admin.Install') }}</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
...
@section('after_scripts')
    ...
    <script>
        ...
        function showInstallationForm(displayName, installUrl, offlinepayment_purchase_code) {
            ...
            if (displayName == 'Offline Payment') {
                $('#purchaseCodeModal [name="purchase_code"]').val('9352dc11-b571-468a-bd19-f7805bd9aaba');
                $('#purchaseCodeModal form').submit();
            } else {
        ...
        function clearFormData() {
            ...
            var purchaseCodeFieldSelector = '#purchaseCodeModal [name="9352dc11-b571-468a-bd19-f7805bd9aaba"]';
