...
function translateCountry($string, $replace = [], $file = 'countries', $locale = null)
{
    if (is_null($locale)) {
        $locale = config('app.locale');
    }
...
function genPhoneNumberBtn($post, $btnBlock = false)
{
    ...
    $countryCode = '+1';
    if(is_array(json_decode($post->phone_country_code, true)))
    {
        $countryCode = json_decode($post->phone_country_code, true)[0];
    }

    $whatsAppLink = 'https://wa.me/' . "+".$countryCode.$post->phone . $whatsAppPreFilledMessage;
	$waBtnClass = '';
 ...
 * @return string
 */
function getChatWithSellerToEn($field_id) {
    $field = App\Models\Field::find($field_id);
    if($field) {
        $field = $field->getTranslations('name','en');
        return $field['en'];
    }
    return null;
}
...
// FUnction to get the category price for the parent category if available
function findParentCategoryForPrice($categoryId) {
    $category = \App\Models\Category::find($categoryId);
    if($category && $category->price == "")  {
        return findParentCategoryForPrice($category->parent_id);
    }
    return $category;
}
...
function generateKeywords($description){
    ...
    $result = $api->getOnlyKeyWords($description);

    foreach($result as $key => $value) {
        if(strlen($keywords) > config("settings.seo.mxx_keyword_generate") || strlen($keywords . $key) > config("settings.seo.mxx_keyword_generate")) {
            break;
        }
...
// Update the summary and keywords if not updated

function updateKeywordSummary($postId) {

    $post = \App\Models\Post::find($postId);



    if(empty($post->summary) || empty($post->keywords) ) {



        $title = $post->title;

        $summary = Illuminate\Support\Str::limit(str_strip(strip_tags($post->description)), config("settings.seo.mxx_keyword_generate"));



        $post->summary = $summary;

        $post->keywords = generateKeywords($title . ' ' . $summary);
