...
class User extends BaseUser
{
    ...
    */
    ...
    {
        return (auth()->check() && auth()->user()->primary_email) ? auth()->user()->primary_email : $this->email;
    }
    ...
    */
    ...
    public function getEmailHtml()
    {
        ...
        $out = '';
        $url = admin_url('user-primary-email/' . $this->id);
    ...
    public function getCompanyHtml()
    {
        ...
        if ($this->company) {
            $url1 = admin_url('view-company/' . $this->company->id);
            $url2 = admin_url('edit-company/' . $this->company->id);
            $out .= "<a href='" . $url1 . "' style='margin-left:5px' class='btn btn-sm btn-light data-toggle='tooltip' data-bs-placement='top' title='View''> <i class='fas fa-eye'></i> </a>";
            $out .= "<a href='" . $url2 . "' style='margin-left:10px' class='btn btn-sm btn-light' data-toggle='tooltip' data-bs-placement='top' title='Edit''> " . $this->company->name . " | <i class='far fa-edit'></i> </a>";
        } else {
            return "<span style='margin-left:65px'> - </span>";
        }
        return $out;
    }
