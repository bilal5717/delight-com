...
use App\Rules\MinumumCategoryPrice;
...
class PostRequest extends Request
{
    ...
    public function validatedDescription()
    {
        return [
            'description' => $this->sanitized('description'),
        ];
    }
    ...
    public function sanitized($type)
    {
        ...
        $cleanInput = preg_replace('/\b(SELECT|UPDATE|DELETE|INSERT|CREATE|DROP|TABLE)\b/i', '', htmlspecialchars_decode($this->input($type)));

        // Remove client-side script injection (e.g., <img src="invalidimage.jpg" onerror="alert('XSS')">)
        $cleanInput = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', '', $cleanInput);

        // Remove event attributes (e.g., onerror) from HTML tags
        $cleanInput = preg_replace('/\s+on\w+="[^"]*"/i', '', $cleanInput);

        // You can add more custom sanitization logic here as needed

        $cleanInput = $purifier->purify($cleanInput);
