...
class SetLocaleController extends FrontController
{
    ...
    /**
     * Handle language switching with path parameter
     *
     * @param string $path
     * @param string $code
     * @return \Illuminate\Http\RedirectResponse
     */
    public function redirectWithPath($path, $code)
    {
        // Check if the selected Language Code is supported by the system
        if (!isAvailableLang($code)) {
            $message = t('language_not_supported', ['code' => $code]);
            flash($message)->error();
            return redirect()->back();
        }
        ...
        // Check if Language Code can be saved into the session
        // and remove the Language Code session, if its cannot be saved.
        $langCanBeSaved = $this->checkIfLangCanBeSavedInSession($code);
    
        // Save the Language Code in Session
        $this->saveLangInSession($code, $langCanBeSaved);
