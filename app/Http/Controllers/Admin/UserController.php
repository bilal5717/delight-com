...
class UserController extends PanelController
{
	...
	public function setup()
	{
		...
		*/
		...
		$this->xPanel->addFilter(
			...
			'',
			function ($value) {
				$this->xPanel->query = $this->xPanel->query->whereHas('company', function ($query) use ($value) {
		...
		*/
		if (request()->segment(2) != 'account') {
			...
			$this->xPanel->addColumn([
				'label'         => mb_ucfirst(trans('company-admin.company_details')),
				'name'          => 'company',
				'type'          => 'model_function',
				'function_name' => 'getCompanyHtml',
			]);
			$this->xPanel->addColumn([
				'name'          => 'verified_email',
	...
	public function userPrimaryEmail($id)
	{
		$user = User::with('company')->find($id);
		return view('admin::panel.user_primary_email')->with('user', $user);
	}
	public function updateEmail(Request $request, $id)
	{
		$user = User::find($id);
		$user->update(['primary_email' => $request->primary_email]);
