...
class PostController extends PanelController
{
	...
	public function setup()
	{
		...
		*/
        ...
        $this->xPanel->addField([
            ...
            'name'              => 'phone_country_code', // The name of the field in your database
            'type'              => 'select2_from_array',
            'options'           => $this->phoneCountryCodes(), // Call the method to get options
            'allows_null'       => false,
            'wrapperAttributes' => [
        ...
        $this->xPanel->addField([
            'label'             => trans('admin.Product Type'),
            'name'              => 'product_type_id', // The name of the field in your database
            'type'              => 'select2_from_array',
            'options'           => $this->productTypes(), // Call the method to get options
            'allows_null'       => false,
            'wrapperAttributes' => [
                'class' => 'form-group col-md-6',
            ],
    ...
    {
        $countryEntries = Country::query()->get(['code', 'phone', 'name']);

        $entries = collect($countryEntries)->mapWithKeys(function ($item) {
            return [
                json_encode([
                    $item['phone'],
                    $item['code']
