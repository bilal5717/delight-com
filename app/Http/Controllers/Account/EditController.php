...
class EditController extends AccountBaseController
{
    ...
    public function workingHoursSettings(Request $request)
    {
        // Get User
        $user = User::find(auth()->user()->id);

        if($request->method() == 'POST'){
            if ($request->filled('working-hours-input')) {
                $user->working_hours = $request->input('working-hours-input');
                $user->working_hours_active = $request->input('working-hour-check');
            }
            $user->save();
        }
        $data = [];
        ...
        $data['working_hours_active'] = $user->working_hours_active;

        // Meta Tags
        MetaTag::set('title', t('my_account'));
        MetaTag::set('description', t('my_account_on', ['appName' => config('settings.app.app_name')]));
        $data['pagePath'] = 'working-hours';

        return appView('account.working-hours', $data);
    }
