		...
		if (isset($messageArray['post_id'], $messageArray['from_email'], $messageArray['from_name'], $messageArray['body'])) {
			try {
				if (!isDemo()) {
					...
					Notification::route('mail', $post->user->primary_email ? $post->user->primary_email : $post->user->email)->notify(new SellerContacted($post, $messageArray));
				}
			} catch (\Exception $e) {
				$errorFound = true;
			...
			try {
				...
				if (isset($thread->users) && $thread->users->count() > 0) {
					foreach ($thread->users as $user) {
						...
						Notification::route('mail', $user->primary_email ? $user->primary_email : $user->email)->notify(new ReplySent($messageArray));
					}
				}
			} catch (\Exception $e) {
				$errorFound = true;
