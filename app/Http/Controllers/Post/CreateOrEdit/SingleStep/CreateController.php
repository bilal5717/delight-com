...
class CreateController extends FrontController
{
    ...
    public function commonQueries()
    {
        ...
        // Get Product Types
        $cacheId = 'productTypes.all.' . config('app.locale');
        $data['productTypes'] = Cache::remember($cacheId, $this->cacheExpiration, function () {
            return ProductType::orderBy('id')->get();
        });
        view()->share('productTypes', $data['productTypes']);

        // Get Countries Data
        $data['countriesData'] = Country::withoutGlobalScopes()->select('name', 'code', 'phone')->orderBy('id')->get();
        view()->share('countriesData', $data['countriesData']);
    ...
    public function postForm(PostRequest $request)
    {
        ...
        if (auth()->check()) {
            $emailVerificationRequired = config('settings.mail.email_verification') == 1
                ...
                && ($request->input('email') != auth()->user()->email && $request->input('email') != auth()->user()->primary_email);
        ...
        foreach ($input as $key => $value) {
            ...
            if($key == 'phone'){
                $phonenumber = strtolower(preg_replace('/-+/', '', preg_replace('/[^\wáéíóú]/', '-', $value)));
                $post->{$key} = ltrim($phonenumber, '0');
            }

            if($key == 'description'){
                $post->{$key} = clean($request->validatedDescription()[$key]);
            }

            if($key == 'title'){
                $post->{$key} = $request->validatedTitle()[$key];
            }
        }

        $keywords = "";
        if (isset($input['post_type_id']) && $input['post_type_id'] != 1) {

            $category_price = findParentCategoryForPrice(request()->category_id);

            if ($input['price'] < $category_price->price) {
                $msg = $category_price->price ."" . config('selectedCurrency.symbol') . " (".$category_price->price . config('selectedCurrency.code') . " " . config('selectedCurrency.name') . ")";
                return redirect()->back()->withErrors([trans('company.invalid_price', ['price_currency' => $msg])])->withInput();
            }

            $value = trim(mb_strtolower($input['description']));
                    ...
                    if($word->entry != ')))')
                    {
                        ...
                        try {
                            if (preg_match('|[' . $startPatten . '\\\]+' . $word->entry . '[' . $endPatten . '\\\]+|ui', ' ' . $value . ' ')) {
                                return redirect()->back()->withErrors([trans('company.can_not_use_bad_word_in_description', ['badword' => $word->entry])])->withInput();
                            }
                        } catch (\Exception $e) {
                            if (preg_match('|[' . $startPatten . ']+' . $word->entry . '[' . $endPatten . ']+|ui', ' ' . $value . ' ')) {
                                return redirect()->back()->withErrors([trans('company.can_not_use_bad_word_in_description', ['badword' => $word->entry])])->withInput();
            ...
            $pattern = '/[\p{Han}\p{Hiragana}\p{Katakana}]+/u'; // Unicode character classes for CJK characters
            if(preg_match($pattern, $desc)){
                $desc_len = Str::length($desc, "UTF-8");
            }
            else{
                $desc_len = count(preg_split('/\s+/', strip_tags($desc)));
            }
            
            if ($desc_len > config('settings.single.max_word_description')) {
                return redirect()->back()->withErrors([trans('company.description_length_not_greater_then')])->withInput();
            }
            if ($desc_len < config('settings.single.min_word_description')) {
                return redirect()->back()->withErrors([trans('company.description_length_not_less_then')])->withInput();
            }

            $title = Str::length($request->input('title'), "UTF-8");

            if ($title >= config('settings.single.title_character_limit')) {
                return redirect()->back()->withErrors([trans('company.title_length_not_greater_then')])->withInput();
            }
            if ($title <= config('settings.single.title_character_minimum_limit')) {
                return redirect()->back()->withErrors([trans('company.title_length_not_less_then')])->withInput();
            }
        ...
        $post->product_type_id = $request->input('product_type_id');
        $post->phone_country_code = $request->input('phone_country_code');
            ...
            if (config('settings.mail.confirmation') == 1) {
                try {
                    if (config('settings.single.posts_review_activation') == 1) {
                        ...
                        Notification::route('mail', $post->user->primary_email ? $post->user->primary_email : $post->user->email)->notify(new PostActivated($post));
                    } else {
                        // $post->notify(new PostReviewed($post));
                        Notification::route('mail', $post->user->primary_email ? $post->user->primary_email : $post->user->email)->notify(new PostReviewed($post));
                    }
                } catch (\Exception $e) {
                    flash($e->getMessage())->error();
                }
