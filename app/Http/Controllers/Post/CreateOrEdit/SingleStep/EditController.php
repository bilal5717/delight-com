...
class EditController extends FrontController
{
    ...
    public function commonQueries()
    {
        ...
        // Get Product Types
        $data['productTypes'] = ProductType::orderBy('id')->get();
        view()->share('productTypes', $data['productTypes']);

        // Get Countries Data
        $data['countriesData'] = Country::withoutGlobalScopes()->select('id', 'name', 'code', 'phone')->orderBy('id')->get();
    ...
    public function postForm($postId, PostRequest $request)
    {
        ...
        // Get the country code input
        $country_code = $request->input('phone_country_code');

        // Decode the country code
        $decoded_country_code = json_decode($country_code, true);

        // Check if decoding was successful and if it's an array
        if (json_last_error() === JSON_ERROR_NONE && is_array($decoded_country_code) && isset($decoded_country_code[0])) {
            $country_code_value = $decoded_country_code[0];
        } else {
            // Handle the error case, e.g., set a default value or throw an exception
            $country_code_value = ''; // or some default value
            // We could also log an error or throw an exception if needed
            // throw new \Exception("Invalid country code JSON");
        }
        ...
        foreach ($input as $key => $value) {
            ...
            if($key == 'phone'){
                $phonenumber = preg_replace('/^(\++)/', '+', preg_replace('/[^\d]/', '', $value));
                $phonenumber = ltrim($phonenumber, '0');
                $post->{$key} = $this->removeCountryCode($phonenumber, $country_code_value);
            }
    ...
    private function removeCountryCode($phoneNumber, $countryCode) {
        // Remove leading '+' if present
        $countryCode = ltrim($countryCode, '+');

        // Construct regular expression pattern to match the country code
        $pattern = '/^(\+?' . preg_quote($countryCode, '/') . ')?\s*/';

        // Replace the country code with an empty string
        $result = preg_replace($pattern, '', $phoneNumber);
