...
class DetailsController extends FrontController
{
    ...
    public function index($postId)
    {
        ...
        $AllActiveAddons = PostAddon::where('post_id', $postId)
        ->where('status' , 'active')
        ->get();
        // GET POST'S DETAILS
        if (auth()->check()) {
        ...
        $company_address = $post->user && $post->user->company && $post->user->company->defaultCompanyAddresss ? $post->user->company->defaultCompanyAddresss : "";
        ...
        if (!empty($post->keywords)) {
            MetaTag::set('keywords', str_replace(',', ', ', $post->keywords));
        ...
        */
        ...
        $posts = Post::getLatestOrSponsored($maxItems, $type, $orderBy, 1, 1, 1)->shuffle();
        ...
        // View
        return view('post.details', compact('AllActiveAddons', 'data'));
