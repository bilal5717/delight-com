...
trait MakeBulkPaymentTrait
{
	...
	{
		// Set URLs
		$this->uri['previousUrl'] = str_replace(['#entryId'], [request()->ids], $this->uri['previousUrl']);
		// Get Payment Method
		...
		if (!empty($paymentMethod)) {
			...
			if (!empty($plugin)) {
				// Send the Payment
				try {
					return call_user_func($plugin->class . '::upgradePosts', $request, $post);
				} catch (\Exception $e) {
	...
	public function paymentConfirmation($postIdOrToken = null)
	{
		...
		// Payment using the selected Payment Method
		try {
			return call_user_func($plugin->class . '::upgradePostsPaymentConfirmation', $params, $post);
		} catch (\Exception $e) {
			flash($e->getMessage())->error();
